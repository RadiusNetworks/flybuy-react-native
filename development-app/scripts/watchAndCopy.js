...
let sourceFolderPath = path.join(
  __dirname,
  `../node_modules/@radiusnetworks/react-native-flybuy-${packageName}`,
);
let destinationFolderPath = path.join(
  __dirname,
  `../../mono/packages/${packageName}`,
);
if (platform) {
...
watcher.on('change', filePath => {
  ...
  execSync(
    `cp -R ${path.join(sourceFolderPath, '*')} ${destinationFolderPath}`,
  );
