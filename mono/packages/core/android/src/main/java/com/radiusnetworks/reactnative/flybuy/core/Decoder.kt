...
fun decodeSiteOptions(options: ReadableMap): SiteOptions {
  return SiteOptions.Builder().apply {
    if (options.hasKey("operationalStatus")) {
      setOperationalStatus(options.getString("operationalStatus") ?: "live")
    }
    if (options.hasKey("page")) {
      setPage(options.getInt("page"))
    }
    if (options.hasKey("per")) {
      setPer(options.getInt("per"))
    }
  }.build()
}
...
fun decodePickupMethodOptions(options: ReadableMap): PickupMethodOptions {
  ...
  return PickupMethodOptions.Builder(pickupType).apply {
    ...
    if (options.hasKey("transportMode")) {
      setTransportMode(options.getString("transportMode"))
    }
...
fun decodeOrderOptions(options: ReadableMap): OrderOptions {
  ...
  return OrderOptions.Builder(customerName).apply {
    if (options.hasKey("customerPhone")) {
      setCustomerPhone(options.getString("customerPhone"))
    }
    if (options.hasKey("customerCarType")) {
      setCustomerCarType(options.getString("customerCarType"))
    }
    ...
    if (options.hasKey("customerLicensePlate")) {
      setCustomerCarPlate(options.getString("customerLicensePlate"))
    }
    if (options.hasKey("partnerIdentifier")) {
      setPartnerIdentifier(options.getString("partnerIdentifier"))
    }
    if (options.hasKey("pickupWindow")) {
      setPickupWindow(decodePickupWindow(options.getMap("pickupWindow")!!))
    }
    if (options.hasKey("state")) {
      setState(options.getString("state"))
    }
    if (options.hasKey("pickupType")) {
      setPickupType(options.getString("pickupType"))
    }
    if (options.hasKey("spotIdentifier")) {
      setSpotIdentifier(options.getString("spotIdentifier"))
    }
    if (options.hasKey("handoffVehicleLocation")) {
      setHandoffVehicleLocation(options.getString("handoffVehicleLocation"))
    }
    ...
    if (options.hasKey("loyaltyIdentifier")) {
      var loyaltyIdentifier = options.getString("loyaltyIdentifier")
      var loyaltyProvider = ""
      if (options.hasKey("loyaltyProvider")) {
        loyaltyProvider = options.getString("loyaltyProvider")!!
      }
      setLoyaltyInfo(loyaltyIdentifier!!, loyaltyProvider!!)
    }
    if (options.hasKey("transportMode")) {
      setTransportMode(options.getString("transportMode"))
    }
...
fun decodeSite(site: ReadableMap): Site {
  ...
  if (site.hasKey("distanceMeters")) {
    distanceMeters = site.getInt("distanceMeters")
  }
